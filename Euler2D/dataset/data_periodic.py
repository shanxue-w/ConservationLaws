...
Boundary conditions follow ``conslaw.solver.FD_WENOZ`` / ``FD_WENOZ2D`` (e.g. ``periodic`` =
edge extrapolation of ghosts).

Dataset generation (``build_euler2d_quadrant_split`` / ``python -m Euler2D.dataset.data``):
evolve on a fine grid with **periodic** BC. By default each random trajectory uses a mixed
IC sampler: with probability 0.8 it samples **four independent random** quadrant states
(log-uniform ``ρ,p``, uniform ``u,v``), and with probability 0.2 it samples a periodic
smooth field built from a few low-frequency Fourier modes. Use ``--fixed_quadrant_ic``
...
DEFAULT_P_RANGE = (0.08, 1.0)
DEFAULT_UV_RANGE = (-1.0, 1.0)

# Random quadrant primitives: enforce |u|/c < this (each quadrant independent).
QUADRANT_MACH_MAX = 3.5
RANDOM_SMOOTH_PROB = 0.2
SMOOTH_MODE_MAX = 3
SMOOTH_MODE_COUNT_RANGE = (2, 4)
SMOOTH_LOG_RHO_AMP_RANGE = (0.08, 0.22)
SMOOTH_LOG_P_AMP_RANGE = (0.08, 0.22)
SMOOTH_VEL_AMP_RANGE = (0.05, 0.35)
...
def _random_low_frequency_fourier_sum(
    XX: np.ndarray,
    YY: np.ndarray,
    rng: np.random.Generator,
    *,
    amp_range: tuple[float, float],
    n_modes_range: tuple[int, int] = SMOOTH_MODE_COUNT_RANGE,
    k_max: int = SMOOTH_MODE_MAX,
) -> np.ndarray:
    """Periodic zero-mean random field built from a few low-frequency Fourier modes."""
    if k_max < 1:
        raise ValueError(f"k_max must be >= 1, got {k_max}.")
    n_lo, n_hi = int(n_modes_range[0]), int(n_modes_range[1])
    if n_lo <= 0 or n_hi < n_lo:
        raise ValueError(f"Invalid n_modes_range={n_modes_range}.")
    candidates = [(kx, ky) for kx in range(k_max + 1) for ky in range(k_max + 1) if (kx, ky) != (0, 0)]
    if not candidates:
        raise ValueError("No nonzero Fourier modes available.")
    n_modes = int(rng.integers(n_lo, n_hi + 1))
    n_modes = min(n_modes, len(candidates))
    field = np.zeros_like(XX, dtype=np.float64)
    picks = rng.choice(len(candidates), size=n_modes, replace=False)
    for idx in np.atleast_1d(picks):
        kx, ky = candidates[int(idx)]
        amp = float(rng.uniform(float(amp_range[0]), float(amp_range[1])))
        phase = float(rng.uniform(0.0, 2.0 * math.pi))
        use_sin = bool(rng.integers(0, 2))
        arg = 2.0 * math.pi * (kx * XX + ky * YY) + phase
        basis = np.sin(arg) if use_sin else np.cos(arg)
        field += amp * basis
    field -= np.mean(field)
    return field
...
def random_smooth(
    ...
    *,
    ...
    """
    Random periodic smooth IC from a few low-frequency Fourier modes in primitive variables.

    ``rho`` and ``p`` are generated in log-space so positivity is automatic.
    """
    if v_range is None:
        v_range = u_range
    xl, xr = map(float, xlim)
    yb, yt = map(float, ylim)
    dx = (xr - xl) / nx
    dy = (yt - yb) / ny
    xc = xl + (np.arange(nx, dtype=np.float64) + 0.5) * dx
    yc = yb + (np.arange(ny, dtype=np.float64) + 0.5) * dy
    XX, YY = np.meshgrid((xc - xl) / (xr - xl), (yc - yb) / (yt - yb), indexing="xy")

    rho_mean = float(np.exp(rng.uniform(np.log(float(rho_range[0])), np.log(float(rho_range[1])))))
    p_mean = float(np.exp(rng.uniform(np.log(float(p_range[0])), np.log(float(p_range[1])))))
    u_mean = float(rng.uniform(float(u_range[0]), float(u_range[1])))
    v_mean = float(rng.uniform(float(v_range[0]), float(v_range[1])))

    log_rho = np.log(rho_mean) + _random_low_frequency_fourier_sum(
        XX, YY, rng, amp_range=SMOOTH_LOG_RHO_AMP_RANGE
    )
    log_p = np.log(p_mean) + _random_low_frequency_fourier_sum(
        XX, YY, rng, amp_range=SMOOTH_LOG_P_AMP_RANGE
    )
    u_vel = u_mean + _random_low_frequency_fourier_sum(XX, YY, rng, amp_range=SMOOTH_VEL_AMP_RANGE)
    v_vel = v_mean + _random_low_frequency_fourier_sum(XX, YY, rng, amp_range=SMOOTH_VEL_AMP_RANGE)

    rho = np.exp(log_rho)
    p = np.exp(log_p)

    c = np.sqrt(np.maximum(gamma * p / np.maximum(rho, RHO_MIN), 1e-30))
    speed = np.sqrt(u_vel * u_vel + v_vel * v_vel)
    mach_field = speed / np.maximum(c, 1e-30)
    mach_peak = float(np.max(mach_field))
    if mach_peak > mach_max and mach_peak > 0.0:
        vel_scale = mach_max * (1.0 - 1e-9) / mach_peak
        u_vel *= vel_scale
        v_vel *= vel_scale

    u0 = primitives_to_conserved_2d(rho, u_vel, v_vel, p, gamma=gamma)
    return u0.astype(np.float64), dx, dy


def sample_random_initial_condition(
    nx: int,
    ny: int,
    rng: np.random.Generator,
    *,
    ...
    u_range: tuple[float, float] = DEFAULT_UV_RANGE,
    v_range: tuple[float, float] | None = None,
    smooth_prob: float = RANDOM_SMOOTH_PROB,
) -> tuple[np.ndarray, float, float, str]:
    """
    Random IC mixture: piecewise-constant quadrant with probability ``1-smooth_prob``,
    periodic smooth Fourier IC with probability ``smooth_prob``.
    """
    if not (0.0 <= smooth_prob <= 1.0):
        raise ValueError(f"smooth_prob must lie in [0, 1], got {smooth_prob}.")
    if rng.uniform() < smooth_prob:
        u0, dx, dy = random_smooth(
            nx,
            ny,
            rng,
            xlim=xlim,
    ...
    return u0, dx, dy, "random_quadrant"
...
def _build_euler2d_quadrant_meta(
    *,
    ...
    return {
        ...
        "nx_fine": int(nx_fine),
        "ny_fine": int(ny_fine),
        "ic": "mixed_random_ic" if random_quadrant_states else "quadrant_riemann",
        "x0_default": float(x0),
        "y0_default": float(y0),
        "random_split": bool(random_split),
        "split_margin": float(split_margin),
        "random_quadrant_states": bool(random_quadrant_states),
        "random_smooth_probability": float(RANDOM_SMOOTH_PROB),
        ...
        "reference": "quadrant IC + fine periodic WENO + cell-average downsample",
        "cfl": float(cfl),
        "gamma": float(GAMMA),
...
def _prepare_euler2d_quadrant_split_stream(
    *,
    ...
    traj_dir = os.path.join(out_dir, f"{split_name}_periodic_trajectories")
    os.makedirs(traj_dir, exist_ok=True)
    if manifest_name is None:
        manifest_name = f"{split_name}_periodic_manifest.pt"
    ...
    jobs = []
    for t in range(n_ic):
        out_path = os.path.join(traj_dir, f"{split_name}_periodic_traj_{t:06d}.pt")
        jobs.append(
            {
                **base_job,
                "traj_idx": t,
...
def _periodic_manifest_name_from_output_name(output_name: str) -> str:
    """Derive a manifest filename that always includes the periodic tag."""
    stem, _ext = os.path.splitext(str(output_name))
    if "periodic" in stem:
        return f"{stem}_manifest.pt"
    return f"{stem}_periodic_manifest.pt"
...
def _build_euler2d_quadrant_chunk(
    ...
    """
    Fine-grid FD-WENOZ (periodic) + mixed random/classic IC -> coarse trajectory states.
    ...
    for s in range(n_ic):
        ...
        if random_quadrant_states:
            u0_fine, _, _, ic_kind = sample_random_initial_condition(
                nx_fine,
                ny_fine,
                rng,
                xlim=(0.0, lx),
...
def build_euler2d_quadrant_splits_to_dir(
    *,
    ...
    for split_name, n_ic, seed, fname in split_specs:
        if n_ic <= 0:
            continue
        manifest_name = _periodic_manifest_name_from_output_name(fname)
        plan = _prepare_euler2d_quadrant_split_stream(
            out_dir=out_dir,
            split_name=split_name,
            n_ic=n_ic,
...
def run_quadrant_periodic_demo(
    nx: int = 128,
    ny: int = 128,
    dt_snap: float = 1e-2,
    t_final: float = 1.0,
    ...
    *,
    ...
    """
    Test: [0,1]^2 periodic IC, save state plots every ``dt_snap`` until ``t_final``.

    With ``clean_export=True``, the default output directory is ``quadrant_periodic_demo_clean``
    ...
    By default, figures are saved as **PDF** 2x2 panels for ``(rho, rho*u, rho*v, E)``
    (vector axes, colorbar, text; the colormap fields may be rasterized inside the PDF).
    With ``clean_export``, each frame is **PDF + PNG** with a minimal
    ...
    Set ``random_quadrant_ic=True`` to sample the dataset's random IC mixture:
    80% piecewise-constant quadrants and 20% periodic smooth Fourier modes.
    ``ic_seed=None`` uses a nondeterministic RNG; pass an int for reproducibility.
    ...
    if random_quadrant_ic:
        if rng is None:
            rng = np.random.default_rng(ic_seed) if ic_seed is not None else np.random.default_rng()
        u0, dx, dy, ic_kind = sample_random_initial_condition(
            nx,
            ny,
            rng,
            xlim=(0.0, 1.0),
    ...
    if outdir is None:
        default_sub = (
            "quadrant_periodic_demo_clean" if clean_export else "quadrant_periodic_demo_2x2"
        )
    ...
    op = build_euler2d_solver(bc="periodic", reconstruction=reconstruction, verbose=verbose)
    if ic_kind == "random_smooth":
        u_note = "random smooth Fourier IC"
    elif ic_kind == "random_quadrant":
        u_note = "random quadrant IC"
    else:
        u_note = "classic U1-U4"
    ...
    print(
        ...
        f"dt_snap={dt_snap}, n_snaps={n_snaps}, bc=periodic, recon={reconstruction}, IC={ic_note}{seed_note}"
    )
    ...
    with open(meta_path, "w", encoding="utf-8") as f:
        ...
        if random_quadrant_ic:
            f.write(f"random_ic_kind={ic_kind}\n")
            f.write(f"random_smooth_probability={RANDOM_SMOOTH_PROB}\n")
...
def _main_dataset_cli():
    ap = argparse.ArgumentParser(
        description="Build Euler2D quadrant-Riemann + periodic datasets (stream per trajectory by default).",
    ...
    ap.add_argument(
        "--random_split",
        action="store_true",
        help="force random (x0,y0) each trajectory (default ON when using random ICs; use --fix_xy to pin)",
    )
    ap.add_argument(
        "--fix_xy",
        action="store_true",
        help="pin (x0,y0) to --x0/--y0 (default when using random ICs: split is random unless this flag)",
    )
    ap.add_argument(
        "--split_margin",
        type=float,
    ...
    ap.add_argument(
            "--fixed_quadrant_ic",
            action="store_true",
            help="use classic fixed U1-U4 benchmark instead of the default random IC mixture (80% quadrant, 20% smooth Fourier)",
    )
    ap.add_argument(
        "--rho_lo",
        type=float,
    ...
    ap.add_argument("--train_name", type=str, default="euler2d_quadrant_periodic_train.pt")
    ap.add_argument("--val_name", type=str, default="euler2d_quadrant_periodic_val.pt")
    ap.add_argument("--test_name", type=str, default="euler2d_quadrant_periodic_test.pt")
    args = ap.parse_args()
    ...
    if args.fixed_quadrant_ic:
        ...
        # Random ICs: random (x0,y0) by default; --fix_xy keeps --x0/--y0.
        if args.fix_xy:
...
if __name__ == "__main__":
    ...
    if argv and argv[0] == "--demo":
        argv = argv[1:]
        ap_demo = argparse.ArgumentParser(description="2D Euler quadrant Riemann + periodic demo")
        ...
        ap_demo.add_argument(
            "--random_ic",
            action="store_true",
            help="use the dataset random IC mixture (80% quadrant, 20% smooth Fourier); default IC is the classic benchmark",
        )
        ap_demo.add_argument(
            "--ic_seed",
            type=int,
        ...
        ap_demo.add_argument(
            ...
            help=(
                ...
                "default output dir is quadrant_periodic_demo_clean (override with --outdir)"
            ),
        )
        ...
        if dargs.clean_export and not outdir:
            outdir = os.path.join(os.path.dirname(__file__), "quadrant_periodic_demo_clean")
        v_rng = None if dargs.v_lo is None or dargs.v_hi is None else (dargs.v_lo, dargs.v_hi)
        run_quadrant_periodic_demo(
            nx=dargs.nx,
            ny=dargs.ny,
