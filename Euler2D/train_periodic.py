...
from conslaw.models import HybridDtStep2d, count_params, maybe_torch_compile, state_dict_for_ckpt
...
def main() -> None:
    ...
    ap.add_argument(
        "--no_zero_mean_rhs",
        action="store_true",
        help="Disable mean-removal projection on the learned rhs.",
    )
    ...
    default_save = default_dt_ckpt_path("hybrid", "periodic")
    ap.add_argument("--save", type=str, default=default_save)
    ...
    project_outflow_rhs = model_bc == "outflow"
    save_path = args.save
    if save_path == default_save:
        save_path = default_dt_ckpt_path("hybrid", model_bc)
    print(
    ...
    print(f"[model] HybridDtStep2d (Euler2D, bc={model_bc}, n_cons=4), params={n_params}")
