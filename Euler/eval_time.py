"""
Time WENO, hybrid, and FNO rollouts for Euler without plotting.
"""
...
from dataset.data import WENO_CFL, conserved_to_primitives, enforce_physical_state, make_cell_centered_grid  # noqa: E402
from eval import DEFAULT_FNO_CKPT, load_optional_fno, rollout_hybrid_conserved, sample_eval_ic, solve_euler_weno_ref  # noqa: E402
...
def primitive_final(traj: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    rho, u_vel, p = conserved_to_primitives(traj[-1])
    return np.asarray(rho), np.asarray(u_vel), np.asarray(p)
...
def save_solution_plot(
    ...
    *,
    ...
    ref_values = primitive_final(ref_traj)
    pred_values = {name: primitive_final(traj) for name, traj in pred_trajs.items()}
    ...
    for idx, ax in enumerate(axes):
        ax.plot(x, ref_values[idx], label="WENO", linewidth=2.0, color="#1f77b4")
        for name, values in pred_values.items():
            ax.plot(x, values[idx], label=names.get(name, name), linewidth=1.8, color=colors.get(name))
...
def main() -> None:
    ...
    ap.add_argument("--ckpt", type=str, default="checkpoints/euler_hybrid_flowmap_dt.pt")
    ...
    ap.add_argument("--outdir", type=str, default="eval_euler_time_out")
    ...
    x, dx = make_cell_centered_grid(args.x_left, args.x_right, args.nx_low)
    pin_mem = device.type == "cuda"

    if args.warmup > 0 and n_steps > 0:
        u_zero = np.zeros((3, args.nx_low), dtype=np.float64)
        u_zero[0] = 1.0
        u_zero[2] = 2.5
    ...
    print(f"[time Euler] dt={dt_snap}, nx={args.nx_low}, cfl={args.cfl}, bc={bc_eval}, n_steps={n_steps}")

    rng = np.random.default_rng(args.seed)
    n_eval = 1 if args.ic_mode == "sod" else args.n_samples
    rows = []
    solution_pack = None
    for sample in range(n_eval):
        u0, ic_type = sample_eval_ic(x, rng, ic_mode=args.ic_mode)
        u0 = enforce_physical_state(u0)
        weno_traj, weno_s = timed_call(device, solve_euler_weno_ref, u0, times, dx, bc=bc_eval, cfl=args.cfl)
        ...
        print(f"[{sample + 1}/{n_eval}] WENO={weno_s:.6g}s hybrid={hybrid_s:.6g}s")
