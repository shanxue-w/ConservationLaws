"""
Rollout 1D SWE hybrid fixed-step map vs WENO reference; optional FNO baseline (single WENO ref).
"""
...
_SWE_ROOT = os.path.abspath(os.path.dirname(__file__))
_REPO_SRC = os.path.join(os.path.dirname(_SWE_ROOT), "src")
...
def solve_swe_weno_ref(state0_fine, times, dx_fine, bc="periodic", cfl=0.4):
    solver = build_swe_solver(bc=bc, reconstruction="characteristic")
...
def block_average_swe(snaps_hi, upsample):
    nT = snaps_hi.shape[0]
    return np.stack(
        [
            np.stack([downsample_cell_average(snaps_hi[j][k], upsample) for k in range(2)], axis=0)
            for j in range(nT)
        ],
        axis=0,
    )
...
def primitive_traj_swe(traj: np.ndarray) -> dict[str, np.ndarray]:
    h_list = []
    u_list = []
    for snap in traj:
        h, u = conserved_to_primitives(snap)
        h_list.append(np.asarray(h, dtype=np.float64))
        u_list.append(np.asarray(u, dtype=np.float64))
    return {
...
def save_swe_rollout_visuals(
    ...
    *,
    ...
    ref_primitive = primitive_traj_swe(ref_traj)
    pred_primitive = {name: primitive_traj_swe(traj) for name, traj in pred_trajs.items()}
    labels = {"h": r"$h$", "u": r"$u$"}
    ...
    for key, label in labels.items():
        ...
        save_xt_pdf(
            ...
            ref_values,
            title="",
            path=os.path.join(outdir, f"xt_{key}_ref{suffix}.pdf"),
            cmap="viridis",
        )
        ax_final = axes_final[list(labels.keys()).index(key)]
        ax_err = axes_err[list(labels.keys()).index(key)]
        ax_final.plot(x, ref_values[-1], label="Ref", lw=2)
        for name, primitive in pred_primitive.items():
            pred_values = primitive[key]
            ...
            save_xt_pdf(
                ...
                pred_values,
                title="",
                path=os.path.join(outdir, f"xt_{key}_{name}{suffix}.pdf"),
                cmap="viridis",
            )
            save_xt_pdf(
                x,
                ...
                err_values,
                title="",
                path=os.path.join(outdir, f"xt_error_{key}_{name}{suffix}.pdf"),
                cmap="magma",
                vmin=0.0,
...
def main():
    ...
    ap.add_argument(
        ...
        help="optional FNO1d baseline; WENO ref computed once for both",
    )
    ...
    ap.add_argument("--outdir", type=str, default="eval_swe_hybrid_out")
    ...
    if run_rollout:
        ...
        x_fine, dx_fine = make_periodic_cell_centers(args.x_left, args.x_right, nx_high)
        dx_low = (args.x_right - args.x_left) / nx_low
        x_low = make_periodic_cell_centers(args.x_left, args.x_right, nx_low)[0]
        ...
        print(
            f"[eval SWE hybrid] model_bc={bc_ckpt}, ref_bc={bc_eval}, dt={dt_snap}, "
        ...
        for s in range(args.n_samples):
            state0, ic_type = sample_mixture_ic(x_fine, rng=rng)
        ...
        if l1_fno is not None:
            save_swe["l1_fno"] = l1_fno
            save_swe["linf_fno"] = linf_fno
            rollout_metrics["fno"] = {"l1": l1_fno, "linf": linf_fno}
        np.savez_compressed(os.path.join(args.outdir, "metrics_hybrid.npz"), **save_swe)
        ...
        if one_pack is not None:
            ...
            save_swe_rollout_visuals(args.outdir, x_low, one_pack[0], one_pack[1], pred_trajs, suffix=f"_seed{args.seed}")
