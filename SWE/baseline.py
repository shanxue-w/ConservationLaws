"""
SWE FNO baseline: FNO flux + :class:`HybridFixedStepMap1d` (n_cons=2) or :class:`FNODtStep1d` dt map.
"""
...
def main():
    ...
    ap.add_argument(
        ...
        default="dt",
        choices=("flux", "dt"),
        help="flux: FNO flux + u-q; dt: FNODtStep1d (n_cons=2).",
    )
    ap.add_argument(
        "--no_zero_mean_rhs",
        action="store_true",
    ...
    ap.add_argument("--save", type=str, default="checkpoints/swe_fno_flowmap_dt_24.pt")
    ...
    save_path = args.save or (
        "checkpoints/swe_fno_baseline_dt.pt" if args.integrator == "dt" else "checkpoints/swe_fno_flux_fixedstep.pt"
    )
    ...
    if args.integrator == "dt":
        ...
        print(f"[model] FNODtStep1d (SWE n_cons=2), modes={args.modes}, params={n_params}")
    else:
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if vm["loss"] < best:
            ...
            if args.integrator == "dt":
                ...
                kind, integ_key = "swe_fno_baseline_dt", "u_plus_dt_rhs"
            else:
                kind, integ_key = "swe_fno_flux_fixedstep", "u_minus_q_projected"
            torch.save(
                {
