"""
Rollout PureConvection fixed-step map vs FD-WENOZ reference (linear advection); optional FNO baseline in one pass.
"""
...
from dataset.data import sample_random_ic

torch.set_default_dtype(torch.float64)

DEFAULT_FNO_CKPT = "checkpoints/pureconvection_fno_flux_fixedstep.pt"
LINE_GRID_ALPHA = 0.3
...
def build_linear_advection_solver(c: float = 1.0):
    def flux(u):
...
def solve_linear_advection_weno_ref(u0_fine, times, dx_fine, c: float = 1.0, cfl=0.4):
    solver = build_linear_advection_solver(c=c)
...
@torch.no_grad()
def rollout_hybrid_scalar(step_model, u0_low, n_steps, device, pin_memory: bool):
    ...
    for _ in range(n_steps):
        ...
        # Break references to cudagraph-managed outputs before reusing them as input.
...
@torch.no_grad()
def evaluate_test_dataset_scalar(step_model, test_pt_path, device, pin_memory: bool, batch_size: int):
...
def main():
    ...
    ap.add_argument("--ckpt", type=str, default="checkpoints/pureconvection_hybrid_fixedstep.pt")
    ap.add_argument(
        "--ckpt_fno",
        type=str,
        default=DEFAULT_FNO_CKPT,
        help="optional FNO1d baseline; WENO ref computed once",
    )
    ...
    ap.add_argument("--c", type=float, default=None, help="advection speed; default from ckpt or dataset meta")
    ...
    ap.add_argument("--outdir", type=str, default="eval_pureconvection_hybrid_out")
    ...
    if os.path.isfile(train_pt):
        data_meta = torch.load(train_pt, map_location="cpu")["meta"]

    dx_ckpt = float(ckpt.get("dx", 0.0))
    c_ckpt = ckpt.get("c")
    if args.c is not None:
        c_eval = float(args.c)
    elif c_ckpt is not None:
        c_eval = float(c_ckpt)
    else:
        if data_meta is not None:
            c_eval = float(data_meta.get("c", 1.0))
            print(f"[eval] c from {train_pt}: {c_eval}")
        else:
            c_eval = 1.0
            print("[eval] c default 1.0 (no ckpt['c'] or train_pv.pt)")
    ...
    if run_rollout:
        ...
        domain = data_meta.get("domain", [0.0, 1.0]) if data_meta is not None else [0.0, 1.0]
        x_left = float(domain[0])
        x_right = float(domain[1])
        length = x_right - x_left
        x_hi = np.linspace(x_left, x_right, nx_high, endpoint=False).astype(np.float64)
        dx_hi = length / nx_high
        dx_low = length / nx_low
        x_low = np.linspace(x_left, x_right, nx_low, endpoint=False)
        ...
        print(
            f"[eval PureConvection] c={c_eval}, dt={dt_snap}, nx_low={nx_low}, upsample={args.upsample}"
        )
        ...
        for s in range(args.n_samples):
            ...
            u0_hi = np.asarray(u0_func(x_hi), dtype=np.float64)
            snaps_hi = solve_linear_advection_weno_ref(u0_hi, times, dx_hi, c=c_eval, cfl=args.cfl)
            ref_low = np.stack([block_average(snaps_hi[j], args.upsample) for j in range(nT)], axis=0)

            u0_low = ref_low[0]
            pred = rollout_hybrid_scalar(model, u0_low, n_steps, device, pin_mem)
            pred_fno = (
                rollout_hybrid_scalar(model_fno, u0_low, n_steps, device, pin_mem)
                if model_fno is not None
                else None
            )
            ...
            print(f"[{s+1}/{args.n_samples}] done ({ic_type})")
        ...
        np.savez_compressed(os.path.join(args.outdir, "metrics_pureconvection.npz"), **save_kw)
        ...
        if one_pack is not None:
            tt, ref_traj, pred_traj = one_pack[0], one_pack[1], one_pack[2]
            pred_fno_traj = one_pack[3] if len(one_pack) > 3 else None
    ...
    if run_test:
        ...
        pred_h, target = evaluate_test_dataset_scalar(model, test_path, device, pin_mem, args.test_batch)
        test_rows = [dataset_error_row("hybrid", pred_h, target)]
        if model_fno is not None:
            pred_f, _ = evaluate_test_dataset_scalar(model_fno, test_path, device, pin_mem, args.test_batch)
