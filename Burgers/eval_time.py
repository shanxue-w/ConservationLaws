"""
Time WENO, hybrid, and FNO rollouts for Burgers without plotting.
"""
...
def main() -> None:
    ...
    ap.add_argument("--outdir", type=str, default="eval_burgers_time_out")
    ...
    print(f"[time Burgers] dt={dt_snap}, nx={args.nx_low}, cfl={args.cfl}, bc={bc_eval}, n_steps={n_steps}")

    rng = np.random.default_rng(args.seed)
    rows = []
    solution_pack = None
    for sample in range(args.n_samples):
        u0 = random_fourier_ic(x, kmax=args.kmax, amp_range=(args.amp_min, args.amp_max), decay=args.decay, rng=rng)
        weno_traj, weno_s = timed_call(device, solve_burgers_weno_ref, u0, times, dx, bc=bc_eval, cfl=args.cfl)
        hybrid_traj, hybrid_s = timed_call(device, rollout_hybrid_scalar, model, u0, n_steps, device, pin_mem, dt_model=dt_model)
        rows.extend(
            [
                {"sample": sample, "method": weno_method, "elapsed_s": weno_s, "n_steps": n_steps, "seconds_per_step": weno_s / max(n_steps, 1)},
                {"sample": sample, "method": "hybrid", "elapsed_s": hybrid_s, "n_steps": n_steps, "seconds_per_step": hybrid_s / max(n_steps, 1)},
            ]
        )
        ...
        if model_fno is not None:
            ...
            rows.append({"sample": sample, "method": "fno", "elapsed_s": fno_s, "n_steps": n_steps, "seconds_per_step": fno_s / max(n_steps, 1)})
