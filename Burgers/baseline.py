"""
Burgers FNO baseline: FNO flux map (:class:`FNOFluxBackbone1d` + :class:`HybridFixedStepMap1d`) or
:class:`FNODtStep1d` (``u + dt * FNO``). Matches train integrator / eval loaders.
"""
...
def main():
    ...
    ap.add_argument("--modes", type=int, default=24, help="FNO Fourier modes (FNO trunk)")
    ap.add_argument("--mr_kernel", type=int, default=5, help="unused for FNO trunk; kept for CLI parity with train.py")
    ap.add_argument(
        "--bc",
        type=str,
        default="periodic",
    ...
    ap.add_argument("--spectral_pad", type=int, default=4, help="unused for FNO trunk; kept for CLI parity")
    ap.add_argument("--fno_padding", type=int, default=2, help="replicate pad for FNO trunk when bc=outflow")
    ...
    ap.add_argument(
        ...
        default="dt",
        choices=("flux", "dt"),
        help="flux: FNO flux + u-q; dt: FNODtStep1d.",
    )
    ap.add_argument(
        "--no_zero_mean_rhs",
        action="store_true",
    ...
    ap.add_argument("--save", type=str, default="checkpoints/burgers_fno_flowmap_dt_24.pt")
    ...
    save_path = args.save or (
        "checkpoints/burgers_fno_baseline_dt.pt"
        if args.integrator == "dt"
        else "checkpoints/burgers_fno_flux_fixedstep.pt"
    )
    ...
    if args.integrator == "dt":
        ...
        print(f"[model] FNODtStep1d (Burgers), modes={args.modes}, params={n_params}")
    else:
        ...
        print(f"[model] HybridFixedStepMap1d + FNOFluxBackbone1d (Burgers), modes={args.modes}, params={n_params}")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if vm["loss"] < best:
            ...
            if args.integrator == "dt":
                ...
                kind, integ_key = "burgers_fno_baseline_dt", "u_plus_dt_rhs"
            else:
                kind, integ_key = "burgers_fno_flux_fixedstep", "u_minus_q_projected"
            torch.save(
                {
